...
const Catalog = () => {
  ...
  useEffect(() => {
    setSelectedGenres(genre ? [genre.toLowerCase()] : []); // Corregido el .toLowerCase()
    setSelectedPlatforms(platform ? [platform.toLowerCase()] : []);
    ...
    const fetchGames = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const filtered = response.data.games.filter(game => {
            const matchesPlatform = platform ? 
              game.platforms.some(p => p.toLowerCase() === platform.toLowerCase()) : true;
            const matchesGenre = genre ? 
              game.genres.some(g => g.toLowerCase() === genre.toLowerCase()) : true;
          ...
          <div className="filter-group">
            ...
            <div className="filter-options">
              {allGenres.slice(0, 8).map(genre => (
                <button
                  key={genre}
                  className={selectedGenres.includes(genre.toLowerCase()) ? 'filter-btn active' : 'filter-btn'}
                  onClick={() => handleGenreChange(genre.toLowerCase())}
                >
                  {genre}
                </button>
              ))}
